$(document).ready(function () {
    ...
    */
    $('#refno').keyup(function () {
        ...
        $.get("/getCheckRefNo", { refno: refno }, (result) => {
            if (result.refno == refno) {
                $("#refno").css("background-color", "red");
                $("#error").text("Reference number already registered");
    ...
    */
    $('#cards').on('click', '.remove', function () {
        ...
        var del = $(this).parent();
        del = del.children()[1].getElementsByClassName('text')[1].innerText

        $(this).parent().remove();
        $.get('/delete', {refno: del}, function(result){
